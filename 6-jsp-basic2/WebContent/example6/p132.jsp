...
<%
	...
	User user2 = new User(2, "John", "Doe", true);
	User user3 = new User(3, "Luke", "Skywalker", false);

	List<User> users = new ArrayList<User>();
	users.add(new User(1, "Tom", "Cruise", true));
	users.add(new User(2, "John", "Doe", true));
	users.add(new User(3, "Luke", "Skywalker", false));
...
<body>
	...
	<!-- 
		1. EL 표현식에서 지역변수, 멤버변수 자원을 사용할 수 없습니다. 
		2. page, request, session, application 객체가 가진 자원만 사용할 수 있습니다.
		3. EL 표현식에서 에러가 발생해도 예외가 전파되지 않습니다.
	-->
	<p>${user1.firstName }</p>
	<p>${user2.firstName }</p>
	<p>${user3.firstName }</p>
	...
	<hr>
	<!-- 
		var=키, value=값 으로 scope=범위 에 저장 
		page, request, session, application 순으로 자원을 찾음
	-->
